...
void setSchedule(const BlynkParam &param, int nSchedule)
{
  ...
  {
    scheduleWeekDay[nSchedule][day % 7] = t.isWeekdaySelected(day);
  } //Time library starts week on Sunday=1, Blynk on Monday=1, need to translate Blynk value to time value!! AND need to run from 0-6 instead of 1-7
}
...
BLYNK_WRITE(V76)
{
  ...
  else if ((step == 4) & (enable == 1))
  {
    delay(50);                               //quick delay
    lcd_blynk.clear();                       //clear lcd screen
    lcd_blynk.print(1, 0, "Place pH meter"); //print out instructions for calibration of pH meter
    lcd_blynk.print(0, 1, "in pH10 solution");
    delay(3000);
    lcd_blynk.clear();
    lcd_blynk.print(3, 0, "When ready ");
    lcd_blynk.print(1, 1, "presses button ");
  ...
  else if ((step == 6) & (enable == 1))
  {
    ...
    if ((ph4val + ph7val) > 11) //if total of pH4 and pH7 reading is greater than 11
    {
      ...
      //Serial.println(offset);                              //set negative to hold value of 1 to change algorithm of pHvalue..offset is < 0
    }
    enable = 0;
    step = -1;
  }
...
void display_lcd(float t, float h, float phValue, int float_sensor, int photoresistor)
{
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Float_sensor:");
  lcd.print(float_sensor + String(" bool"));
  lcd.setCursor(0, 1);
  lcd.print("Photoresistor:");
  lcd.print(photoresistor + String(" bool"));
}
void sendSensor()
{
...
void send_data_to_googlesheet()
{
  ...
  url = "/macros/s/AKfycbywGIJQEbRYQUY7ROeF7jTHdO91HbiNHAQtq9qYDGLn5-obhTZjYQPcJT-oLv0X7_m8/exec?value1=" + String(t) +
        "&value2=" + String(h) + "&value3=" + String(phValue) + "&value4=" + String(test_value) + "&value5=" + String(pred_value) + "&value6=" + String(mae) + "&value7=" + status_pH_sensor;
